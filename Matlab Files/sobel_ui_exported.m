classdef sobel_ui_exported < matlab.apps.AppBase

    % Properties that correspond to app components
    properties (Access = public)
        SobelUIFigure  matlab.ui.Figure
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            % Create SobelUIFigure and hide until all components are created
            app.SobelUIFigure = uifigure('Visible', 'off');
            app.SobelUIFigure.Color = [0.1608 0.1608 0.1686];
            app.SobelUIFigure.Position = [100 60 300 400];
            app.SobelUIFigure.Name = 'Sobel';

            % Create UIAxes
            app.UIAxes = uiaxes(app.SobelUIFigure);
            title(app.UIAxes, '')
            xlabel(app.UIAxes, '')
            ylabel(app.UIAxes, '')
    ...
    methods (Access = public)
        ...
        % Construct app
        function app = sobel_ui_exported(varargin)

            % Create UIFigure and components
            createComponents(app)

            % Register the app with App Designer
            registerApp(app, app.SobelUIFigure)
