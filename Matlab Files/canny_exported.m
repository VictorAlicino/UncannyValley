classdef canny_exported < matlab.apps.AppBase

    % Properties that correspond to app components
    properties (Access = public)
        CannyUIFigure  matlab.ui.Figure
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            % Create CannyUIFigure and hide until all components are created
            app.CannyUIFigure = uifigure('Visible', 'off');
            app.CannyUIFigure.Color = [0.1608 0.1608 0.1686];
            app.CannyUIFigure.Position = [100 60 300 410];
            app.CannyUIFigure.Name = 'Canny';

            % Create UIAxes
            app.UIAxes = uiaxes(app.CannyUIFigure);
            title(app.UIAxes, '')
            xlabel(app.UIAxes, '')
            ylabel(app.UIAxes, '')
    ...
    methods (Access = public)
        ...
        % Construct app
        function app = canny_exported(varargin)

            % Create UIFigure and components
            createComponents(app)

            % Register the app with App Designer
            registerApp(app, app.CannyUIFigure)
